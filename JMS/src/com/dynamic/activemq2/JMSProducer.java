...
public class JMSProducer {
	...
	private static final String BROKEURL = ActiveMQConnection.DEFAULT_BROKER_URL;// 默认地址;
	...
	public static void main(String[] args) {
		...
		//	destination = session.createQueue("FirstQueue1");// 创建消息队列;
			destination=session.createTopic("FirstTopic1"); //创建消息主题
	...
	public static void sendMessage(Session session,
		...
		for (int i = 0; i < JMSProducer.SENDNUM; i++) {
			TextMessage message = session.createTextMessage("ActiveMQ 发送的消息"
					+ i);
			System.out.println("发布的消息" + "ActiveMQ 发送的消息" + i);
